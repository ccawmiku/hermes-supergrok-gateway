"""OpenAI-compatible proxy and password-protected LAN control panel."""
...
def _is_json_request(request: web.Request) -> bool:
...
def create_app(
    ...
    *,
    ...
    async def dashboard_security(
        ...
        browser_path = (
            request.path in {"/", "/dashboard", "/auth"}
        ...
        session_authenticated = web_auth.authenticated(
            request.cookies.get(COOKIE_NAME, "")
        )
        protected_browser_path = (
            request.path in {"/", "/dashboard"}
            or request.path in {"/assets/app.css", "/assets/app.js"}
            or request.path.startswith("/admin/")
        )
        if protected_browser_path and not session_authenticated:
            if request.path.startswith("/admin/api/"):
                return _admin_error(401, "请先登录控制面板", "dashboard_auth_required")
    ...
    async def auth_page(request: web.Request) -> web.Response:
        if web_auth.configured() and web_auth.authenticated(
            request.cookies.get(COOKIE_NAME, "")
        ):
            raise web.HTTPFound("/")
        return web.Response(text=_web_text("auth.html"), content_type="text/html")

    async def auth_status(request: web.Request) -> web.Response:
        return web.json_response(
            {
                "ok": True,
                "configured": web_auth.configured(),
                "authenticated": web_auth.authenticated(
                    request.cookies.get(COOKIE_NAME, "")
                ),
                "requirements": {
                    "minimum_length": 8,
    ...
    def set_session_cookie(
        response: web.StreamResponse, request: web.Request, token: str
    ) -> None:
        response.set_cookie(
            COOKIE_NAME,
            token,
    ...
    async def auth_setup(request: web.Request) -> web.Response:
        if not _same_origin(request):
            return _admin_error(403, "请求来源无效", "origin_rejected")
        if not _is_json_request(request):
            return _admin_error(415, "请使用 JSON 请求", "json_required")
        try:
            ...
            if password != confirmation:
                return _admin_error(400, "两次输入的密码不一致", "password_mismatch")
            await asyncio.to_thread(web_auth.set_initial_password, password)
            token = web_auth.create_session()
            response = web.json_response({"ok": True, "message": "管理密码设置成功"})
            set_session_cookie(response, request, token)
            return response
        except WebAuthError as exc:
            status = 409 if exc.code == "password_already_configured" else 400
            return _admin_error(status, str(exc), exc.code)
        except Exception:
            logger.exception("failed to configure dashboard password")
            return _admin_error(500, "无法保存管理密码", "password_setup_failed")
    ...
    async def auth_login(request: web.Request) -> web.Response:
        ...
        if not web_auth.configured():
            return _admin_error(409, "请先设置管理密码", "setup_required")
        remote = request.remote or "unknown"
        retry_after = web_auth.login_retry_after(remote)
        if retry_after:
            response = _admin_error(
                429, "尝试次数过多，请稍后再试", "login_rate_limited"
            )
        ...
        try:
            ...
            return _admin_error(400, "请求内容无效", "invalid_json")
        if not await asyncio.to_thread(web_auth.verify_password, password):
            web_auth.record_login_failure(remote)
            return _admin_error(401, "密码不正确", "invalid_password")
        web_auth.clear_login_failures(remote)
        token = web_auth.create_session()
        response = web.json_response({"ok": True, "message": "登录成功"})
        set_session_cookie(response, request, token)
        return response
    ...
    async def auth_logout(request: web.Request) -> web.Response:
        if not _same_origin(request):
            return _admin_error(403, "请求来源无效", "origin_rejected")
        web_auth.revoke_session(request.cookies.get(COOKIE_NAME, ""))
        response = web.json_response({"ok": True, "message": "已退出控制面板"})
    ...
    async def asset(request: web.Request) -> web.Response:
        ...
        if filename not in {"app.css", "app.js", "auth.css", "auth.js"}:
    ...
    app.router.add_get("/auth/api/status", auth_status)
    app.router.add_post("/auth/api/setup", auth_setup)
    app.router.add_post("/auth/api/login", auth_login)
    app.router.add_post("/auth/api/logout", auth_logout)
